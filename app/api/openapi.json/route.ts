...
import { readRemoteCredentialToken } from "../../lib/auth/remote-credential";
...
export async function GET(request: Request) {
  ...
  if (identity.role !== "admin" && identity.role !== "owner_admin") return apiError(new ApiHttpError(403, "FORBIDDEN", "API sənədləri yalnız administrator üçündür."));

  try {
    const token = readRemoteCredentialToken(request);
    if (!token) return apiError(new ApiHttpError(401, "UNAUTHENTICATED", "API sənədləri üçün sessiya tapılmadı."));
    const baseUrl = (process.env.EDURATE_API_BASE_URL?.trim() || "https://edurate-api.onrender.com").replace(/\/+$/, "");
    const response = await fetch(`${baseUrl}/api/openapi.json`, { headers: { Authorization: `Bearer ${token}` }, cache: "no-store", signal: AbortSignal.timeout(65_000) });
